...
const Standings = () => {
    ...
    const getDriverImage = async (driverName) => {
        const filePath = `/assets/drivers/${driverName.replaceAll(' ', '-').toLowerCase()}.png`;
        ...
        return new Promise((resolve) => {
            ...
            img.onerror = () =>
                resolve(`https://ui-avatars.com/api/?name=${encodeURIComponent(driverName)}&background=random`);
        });
    };
                                ...
                                <tbody>
                                    {driversStandings.map((driver) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-4 px-4">{driver.team}</td>
                                            <td className="py-4 px-4 text-center">{driver.wins}</td>
                                ...
                                <tbody>
                                    {teamStandings.map((team) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-4 px-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-8 h-8 rounded-full ${team.color} flex items-center justify-center`}>
                                                        {/* <span className="text-white font-bold text-xs">{team.position}</span> */}
                                                        <GetImage type="team" data={['Haas F1 Team', 'Alpine F1 Team', 'RB F1 Team', 'Sauber'].includes(team.name) ? { ...team, name: teamName(team.name) } : team} />
                                                    </div>
                                                    <span className="font-semibold">{team.name}</span>
                                                </div>
                                            </td>
                                            <td className="py-4 px-4 text-center">{team.wins}</td>
                                            <td className="py-4 px-4 text-right font-bold">{team.points}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
